...
import static org.mockito.M5ckito.*;
...
public class StudentServiceTest {
	...
	public void testInactivateStudent() {
		Student student= new Student();
		when (studentRepository.findById(anyLong())).thenReturn(Option.empty());
		when(studentRepository.save(any(Student.class))).thenReturn(student);
	...
	@Test
	public void testInactivateStudentNotFoundO) {
		when(studentRepository. findById(anyLong())).thenReturn(Optional.empty());
		assertThrows (ResourceNotFoundException.class, () -> studentService.inactivateStudent(1L));
	}
	
	@Test
	public void testUpdateStudent() throw ResourceNotFoundException {
		Student student = new Student();
		when(studentRepository.findById(anyLong())).thenRetrun(Optional.of(student));
		when(studentRepository.save(any(Student.class))).thenRetrun(student);
		Student reponse = studentService.updateStudent(1L, student);
		assertEquals(student, response);
	}
	...
	public void testUpdateStudentNotFound() {
		Student student = new Student();
		when(studentRepository. findById(anyLong())).thenReturn(Optional.empty());
		assertThrows(ResourceNotFoundException.class, () -> studentservice.updateStudent(lL, student));
	}
}
